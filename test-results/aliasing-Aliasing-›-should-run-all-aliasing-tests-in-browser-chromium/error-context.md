...
- Name: aliasing.spec.ts >> Aliasing › should run all aliasing tests in browser
- Location: tests\aliasing.spec.ts:3:1
...
```ts
  ...
  2  | 
  3  | test('Aliasing › should run all aliasing tests in browser', async ({ page }) => {
  4  |   // Start local server and navigate
  5  |   await page.goto('http://localhost:8000/tests/aliasing.html');
  6  |   
...
> 8  |   await page.waitForFunction(() => window.testResults !== undefined, { timeout: 5000 });
  ...
  12 |   
  13 |   // Log results
  14 |   console.log(`Aliasing tests: ${results.passed}/${results.total} passed`);
