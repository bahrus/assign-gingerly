...
- Name: can-spawn.spec.ts >> assignGingerly - canSpawn Support >> should run all canSpawn tests in browser
- Location: tests\can-spawn.spec.ts:4:3
...
```ts
  ...
  2  | 
  3  | test.describe('assignGingerly - canSpawn Support', () => {
  4  |   test('should run all canSpawn tests in browser', async ({ page }) => {
  5  |     await page.goto('/tests/can-spawn.html');
  6  |     
> 7  |     await page.waitForFunction(() => {
  ...
  19 |     });
  20 |     
  21 |     console.log(`canSpawn tests: ${results.passed}/${results.total} passed`);
  22 |     
